  ...
  BridgeClient.prototype.waitUntilReady = function (timeoutMs) {
    ...
    return new Promise(function (resolve, reject) {
      var timer = setTimeout(function () {
        ...
        if (idx >= 0) {
          self.readyWaiters.splice(idx, 1);
        }
        reject({
          code: 'BRIDGE_REQUEST_TIMEOUT',
          message: 'Bridge 握手等待超时',
        });
      }, ms);
      function onReady() {
        clearTimeout(timer);
    ...
    if (msg.success) {
      ...
      var err =
        msg.error && typeof msg.error === 'object'
          ? msg.error
          : { code: 'UNKNOWN_ERROR', message: '未知错误' };
      pending.reject(err);
    }
  };
      ...
      request: function (module, action, params) {
        var mocks = {
          ...
          'auth.getToken': function () {
            var token = null;
            try {
              token = global.localStorage.getItem('base_h5:token');
            } catch (e) {}
            return { token: token };
          },
